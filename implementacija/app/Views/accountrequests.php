...
<?php
    for ($i=0; $i<count($requests); $i++) {
        ...
        <div class="d-flex mb-3 justify-content-end">
            <form method="POST" action="'.esc(base_url('AdminController/OPApproveRequest/?id='.esc($req->idKorisnika))).'">
                <button type="submit" class="btn btn-success mx-1">'.lang('App.approve').'</button>
            </form>
            <form method="POST" action="'.esc(base_url('AdminController/OPDenyRequest/?id='.esc($req->idKorisnika))).'">
                <button type="sumbit" class="btn btn-danger mx-1">'.lang('App.deny').'</button>
            </form>
            </div>
        </div>';
    }
